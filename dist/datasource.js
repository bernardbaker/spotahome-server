...
exports.HomeCardsAPI = void 0;

var _apolloDatasourceRest = require("apollo-datasource-rest");

class HomeCardsAPI extends _apolloDatasourceRest.RESTDataSource {
  constructor() {
    super();
  ...
  async getHomeCards(limit = 30) {
    ...
    if (response.data.length > 30) response.data.length = 30; // return the results

    const result = await Promise.all(response.data.map(item => {
...
exports.HomeCardsAPI = HomeCardsAPI;